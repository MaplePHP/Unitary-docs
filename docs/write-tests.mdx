...
Unitary follows a validation-first model: every test begins with an input and a set of expectations. Groups define the subject under test, and each validation describes one of its responsibilities with clear intent.
...
group("Example API Response", function(TestCase $case) {
    ...
    $case->expect($json)
        ...
        ->validate("Checking API response code");
});
```
* `validate()`: You can define a custom heading for the validation.
* `describe()`: Add optional notes to explain the validation and may be chained multiple times when additional clarity is needed.
...
## Hard stops
You may chain `assert()` as the final step, replaces `validate()` to trigger a **hard stop** in the group.
Validations below it will not run, which is helpful when later checks depend on earlier conditions or when you want to avoid cascading errors.
...
group("Example API Response", function(TestCase $case) {
    ...
    $case->expect('{"response":{"status":200}}')
        ->isJson()
        ->hasJsonValueAt("response.status", 200)
        ->assert("Invalid JSON structure");
});
```
_Use hard stops when later checks depend entirely on earlier guarantees._
...
## Advanced usage
Use the callback-based validation when a validation needs staged or conditional logic. It supports **soft stops**, which end only the current validation block. This is ideal for dependent checks or logic that should run only after earlier conditions succeed.
```php
group("1. Soft stop examples", function(TestCase $case) {
    ...
    $case->validate($json, function(Expect $expect) {
        ...
        $expect->isJson()
               ->assert("Invalid JSON"); // soft stop on failure

        $expect->hasJsonValueAt("response.status", 200)
               ->assert("Unexpected status"); // soft stop on failure
    });
