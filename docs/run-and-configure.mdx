...
While CLI flags and the config file define global behavior, individual groups can define their own configuration using `TestConfig`.
This allows naming, skipping, or scoping tests directly in code without affecting the rest of the suite.

```php
use MaplePHP\Unitary\{TestConfig, TestCase, Expect};

$config = TestConfig::make("Checkout API")
    ->withName("checkout")
    ->withSkip(); // temporarily skip this group
...
Configurations are immutable, so you can derive variations without side effects:

```php
group($config->withSubject("Refund flow"), function (TestCase $case) {
    // ...
});
```

Named groups can always be executed in isolation:
